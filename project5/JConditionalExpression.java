...
class JConditionalExpression extends JExpression {
    ...
    public JExpression analyze(Context context) {
        ...
        condition.type().mustMatchExpected(line(), Type.BOOLEAN); // has to be boolean
        thenPart = (JExpression)thenPart.analyze(context);
        elsePart = (JExpression)elsePart.analyze(context);
        elsePart.type().mustMatchExpected(line(), thenPart.type()); // has to be same return type
        type = elsePart.type();
        return this;
    }
    ...
    public void codegen(CLEmitter output) {
        // Modified from JIfstatement, removing the if statements because the labels always are generated
